...
const DEFAULT_SETTINGS = {
  ...
  businessPageUrl: '',
  // Added 2026-09-02, per Greg, for 3D ("Post to Group"): the URL of the
  // specific Facebook group he posts to. Blank by default, same reasoning as
  // businessPageUrl — no sensible universal default, it's per-install (and
  // per-group, if Greg ever runs more than one; a single field is enough for
  // now since only one was asked for).
  groupUrl: '',
  messageTemplates: {
    intro:
  ...
  testMode: true,
  autoSend: false,
  // Added 2026-09-02, per Greg: a "don't touch unless you know what you're
  // doing" section — settings that exist for tuning anti-detection pacing,
  // not day-to-day use. tabCloseMin/MaxSeconds control the randomized pause
  // between a background-tab click (e.g. Add Friend) and that tab actually
  // closing — Greg's own concern was that a single fixed pause (originally
  // hardcoded 1.5-2.5s) would itself become a detectable pattern if every
  // close landed in the same narrow window; a wider, user-adjustable range
  // avoids that without needing a code change to retune it later.
  advanced: {
    tabCloseMinSeconds: 2,
    tabCloseMaxSeconds: 8,
  },
};
