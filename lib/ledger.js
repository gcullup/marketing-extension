...
/**
 * Step 9's foundational transition: someone we sent a friend request to has
 * actually accepted (detected via `checkFriendStatus` on their real profile
 * page — see content/scrape.js). Moves `requested` → `accepted`.
 */
export async function markAccepted(id) {
  const ledger = await readLedger();
  const record = ledger[id];
  if (!record) throw new Error(`No ledger record for ${id}`);
  record.state = STATES.ACCEPTED;
  record.acceptedAt = Date.now();
  appendHistory(record, 'friend request accepted (detected via profile check)');
  ledger[id] = record;
  await writeLedger(ledger);
  return record;
}
...
/**
 * Per Greg's design (2026-09-01): a friend request still outstanding past
 * `settings.staleRequestDays` gets withdrawn rather than sitting forever —
 * the time-based counterpart to the original Step 2 spec's volume-based
 * cleanup. Moves `requested` → `cancelled`.
 */
export async function markCancelled(id) {
  const ledger = await readLedger();
  const record = ledger[id];
  if (!record) throw new Error(`No ledger record for ${id}`);
  record.state = STATES.CANCELLED;
  record.cancelledAt = Date.now();
  appendHistory(record, 'friend request cancelled — outstanding too long with no response');
  ledger[id] = record;
  await writeLedger(ledger);
  return record;
}
...
/**
 * Deletes specific people by id, entirely — not just changing their state.
 * Added 2026-09-01, per Greg's request for a checkbox-based interface to
 * remove individual unwanted entries, rather than only `clearByState`'s
 * blunt "delete everyone in state X." Same irreversibility discipline as
 * `clearByState` — callers must confirm with the user before calling this.
 * Silently ignores any id not actually present, so a stale checkbox
 * selection can't throw.
 */
...
/** Same pattern as countRequestedToday, for settings.caps.maxMessagesPerDay. */
export async function countDmSentToday(now = new Date()) {
  const people = await getAllPeople();
  return people.filter((p) => p.dmSentAt && isSameLocalDay(p.dmSentAt, now)).length;
}

/**
 * Step 9's cohort query, per Greg's simplified design (2026-09-01): accepted
 * friends who've never been sent a greeting DM and have been accepted for
 * MORE than `dmDelayDays` (strictly — exactly on the boundary still waits
 * one more day), sorted oldest-accepted-first so the longest-waiting people
 * get messaged before the front of the line refills with newer acceptances.
 * No separate `dm_queued` transitional state needed — like Step 2/9's
 * original design intent, this is just a filter over the existing table,
 * not a queue that has to be separately populated and kept in sync.
 */
export async function getDmCandidates(dmDelayDays, now = new Date()) {
  const cutoff = now.getTime() - dmDelayDays * 24 * 60 * 60 * 1000;
  const people = await getAllPeople();
  return people
    .filter((p) => p.state === STATES.ACCEPTED && !p.dmSentAt && p.acceptedAt && p.acceptedAt < cutoff)
    .sort((a, b) => a.acceptedAt - b.acceptedAt);
}

/**
 * Records that the Step 9 greeting DM actually went out. Moves `accepted` →
 * `dm_sent`. `message` is stored so a disputed "what did we actually say" is
 * always auditable, matching the discipline already used for AI screening
 * calls.
 */
export async function markDmSent(id, message) {
  const ledger = await readLedger();
  const record = ledger[id];
  if (!record) throw new Error(`No ledger record for ${id}`);
  record.state = STATES.DM_SENT;
  record.dmSentAt = Date.now();
  record.dmMessage = message;
  appendHistory(record, 'greeting DM sent');
  ledger[id] = record;
  await writeLedger(ledger);
  return record;
}
