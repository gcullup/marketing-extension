 ...
 * can attach (e.g. "Labor Day") — saved alongside the draft so reopening the
 * page later still shows what theme produced it, and so Regenerate defaults
 * to the same theme rather than silently dropping it.
 */
export async function saveDraft({ date = new Date(), dayKey, contentType, text, modifier = '' }) {
  const all = await readContent();
