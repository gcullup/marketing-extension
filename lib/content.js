...
function buildPrompt(plan, targetPersona, modifier, recentContent = [], lengthFeedback = '') {
  const lines = [
    plan.guidance,
    '',
...
async function callClaude({ apiKey, model, plan, targetPersona, modifier, recentContent, lengthFeedback }) {
  const response = await fetch(API_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'user', content: buildPrompt(plan, targetPersona, modifier, recentContent, lengthFeedback) }],
    }),
  });
...
export async function generateContent({
  ...
  const attempt = (lengthFeedback) =>
    callClaude({ apiKey, model, plan, targetPersona, modifier, recentContent, lengthFeedback });
  let result;
  try {
  ...
  return { dayKey, plan, ...best, overLimit };
}
