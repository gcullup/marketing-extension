...
// Exposed so the UI can build its override dropdown from the same data
// this module uses internally, rather than hardcoding a second copy of the
// type list/labels that could drift out of sync.
 ...
 */
export function resolveContentPlan(date, overrideType) {
  const { dayKey, plan: dayPlan } = getContentPlanForDate(date);
  if (overrideType) {
...
export async function generateContent({
  ...
  const { dayKey, plan } = resolveContentPlan(date, overrideType);
