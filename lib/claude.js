...
/**
 * Screens one candidate against a target persona. Throws on any failure
 * (missing config, network error, non-2xx response, or a response that
 * fails schema validation) rather than returning a fallback value — callers
 * decide how to handle that, but this function never fabricates a result.
 */
