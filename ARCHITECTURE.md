...
**Scope, confirmed with Greg:** 3A/3B/3C/3D are four *destinations* for the same day's content, not
four pipeline stages — 3A personal page, 3B business page, 3C Story, 3D a group Greg runs. One
shared content plan drives all four to start (the same generated draft gets posted everywhere,
rather than each destination running its own independent calendar) — simpler to build now, worth
revisiting only if a destination genuinely needs its own content later. This explicitly revises D5
(personal-profile-only) for content specifically — friend-request/DM automation (Steps 1 and 9)
stays personal-profile-only as originally decided.

**Day-of-week content calendar, per Greg** (the actual prompt wording is a placeholder — Greg
explicitly wants to engineer this properly later; today's goal was proving the pipeline, not the
prompt):
- Mon/Wed — short-form: must read as complete within Facebook's post preview, no "See More" click
  needed. `SHORT_FORM_MAX_CHARS` (400) is a conservative starting guess at Facebook's real
  truncation point, not yet verified against a real post.
- Tue/Thu — long-form: a photo attached separately by Greg, not generated by the extension.
- Fri — engagement: deliberately unrelated to the target persona/real estate — generic,
  fun questions designed purely to drive comments; a video with a song overlay attached separately
  by Greg.
- Sat/Sun — no plan defined yet, left undefined rather than guessed.

Human review before posting was explicitly requested (matching D6's assisted-click philosophy) — a
public post is far more visible and permanent than a DM, so nothing posts without Greg seeing it
first.

**Built:**
- **`lib/content.js`** — `getContentPlanForDate(date)` (pure day-of-week → content-type lookup,
  verified in a live browser JS sandbox against a full week starting from a known date) and
  `generateContent({apiKey, model, targetPersona, date})`. Its own Claude call, tool schema
  (`draft_post`, just `{content: string}`), and prompt builder — deliberately NOT bolted onto
  `lib/claude.js`'s `screenCandidate`, since screening and content drafting are different concerns
  that happen to both call Claude; same retry-once-on-failure discipline, though. Returns
  `{dayKey, plan: null}` without ever calling Claude on a day with no plan (currently weekends) —
  not an error, just nothing to draft yet.
- **`lib/contentLedger.js`** — one record per calendar date (`draft`/`approved` state, the text, and
  a `postedTo` field for personal/businessPage/story/group, pre-declared now the same way
  `lib/ledger.js`'s `STATES` enum pre-declared DM states early on, even though the actual 3A-3D
  posting actions aren't built yet). `saveDraft` overwrites the whole record (used by both the
  first Generate and any later Regenerate); `approveContent` saves whatever text is currently in the
  box, in case Greg hand-edited the AI's draft, before marking it approved.
- **New side panel page, `sidepanel/content.html`/`content.js`** ("Content") — shows today's content
  type and a media reminder when relevant (photo/video), an editable draft textarea with a live
  character count (flagged over-limit for short-form), Generate (also serves as Regenerate), and
...
| **3** | Step 3 — content creation (generation started; 3A-3D posting not yet built) | Third endpoint — new domain (content, not people), started 2026-09-01. |
| **4** | Steps 2, 4, 6, 7, 8 | Mostly new filters over plumbing that already exists |
